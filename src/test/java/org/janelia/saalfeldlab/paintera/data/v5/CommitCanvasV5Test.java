package org.janelia.saalfeldlab.paintera.data.v5;
...
public class CommitCanvasV5Test {
	...
	public void testSingleScaleLabelMultisetCommit() throws IOException, UnableToPersistCanvas, UnableToUpdateLabelBlockLookup, ReflectionException {
		...
		final V5FSWriter writer = N5TestUtil.fileSystemVersionWriterAtTmpDir(!LOG.isDebugEnabled());
		final var container = new N5ContainerState(writer.getVersionedUrl(), writer, writer);
	...
	private static void testUnsignedLongTypeSingleScale(
		...
		CommitCanvasV5Test.<UnsignedLongType>testSingleScale(
				container,
				dataset,
				DataType.UINT64,
				canvas,
	...
	private static void testLabelMultisetSingleScale(
		...
		CommitCanvasV5Test.testSingleScale(
				container,
				dataset,
				DataType.UINT8,
				canvas,
				ThrowingBiFunction.unchecked(N5LabelMultisets::openLabelMultiset),
				CommitCanvasV5Test::assertMultisetType,
				MULTISET_ATTRIBUTE);
	}
	...
	private static void testUnsignedLongTypeMultiScale(
		...
		CommitCanvasV5Test.<UnsignedLongType>testMultiScale(
				container,
				dataset,
				DataType.UINT64,
				canvas,
	...
	private static void testLabelMultisetMultiScale(
		...
		CommitCanvasV5Test.testMultiScale(
				container,
				dataset,
				DataType.UINT8,
				canvas,
	...
	private static void testUnsignedLongTypePaintera(
		...
		CommitCanvasV5Test.<UnsignedLongType>testPainteraData(
				container,
				dataset,
				DataType.UINT64,
				canvas,
	...
	private static void testLabelMultisetPaintera(
		...
		CommitCanvasV5Test.testPainteraData(
				container,
				dataset,
				DataType.UINT8,
				canvas,
	...
	private static <T> void testPainteraData(
		...
		Grids.forEachOffset(
				...
				Arrays.stream(canvas.getCellGrid().getGridDimensions()).map(x -> x-1).toArray(),
				ones(dims.length),
				ThrowingConsumer.unchecked(b ->
				{
//					System.out.println(Utils.format(writer.getDatasetAttributes(u0).getDimensions())+"-"+Utils.format(new CellGrid(writer.getDatasetAttributes(u0).getDimensions(),writer.getDatasetAttributes(u0).getBlockSize()).getGridDimensions())+"-"+u0+" - "+Utils.format(uniqueAttributes.getDimensions())+" -> "+Utils.format(b));
					writer.writeBlock(u0, uniqueAttributes, new LongArrayDataBlock(new int[]{1}, b, new long[]{}));})
		);
	...
	private static MetadataState getDummyMetadataState(String labelsDataset, N5Writer container) {
		...
		return new MetadataState() {
			...
			@Override
			public void setDatasetAttributes(@NotNull DatasetAttributes datasetAttributes) {

			}

			@Override
			public void setTransform(@NotNull AffineTransform3D transform) {
			...
			@Override
			public void setLabelMultiset(boolean isLabelMultiset) {
			...
			@Override
			public void setPixelResolution(@NotNull double[] pixelResolution) {

			}

			@Override
			public void setOffset(@NotNull double[] offset) {
			...
			@Override
			public void setReader(@NotNull N5Reader reader) {

			}

			@Override
			public void setWriter(@Nullable N5Writer writer) {
			...
			@Override
			public void updateTransform(double[] resolution, double[] offset) {

			}

			@Override
			public void updateTransform(AffineTransform3D newTransform) {
			...
			@Override
			public N5ContainerState getN5ContainerState() {
